 ...
 *      - Với mỗi judge: Tính tổng điểm có trọng số = sum(score × weight) / sum(weight)
 *      - finalScore = trung bình cộng điểm của tất cả các judge
 * 5. Xếp hạng THEO TỪNG HẠng mục (Track) — không xếp chung toàn bộ sự kiện.
 * 6. Áp dụng Admin Override (nếu có) — thay thứ hạng tự động bằng thứ hạng thủ công.
 *
...
public class RankingService {
    ...
    /**
     * EPSILON — Ngưỡng sai số chấp nhận được khi so sánh điểm số kiểu double/BigDecimal.
     * Nếu |scoreA - scoreB| < EPSILON → coi 2 đội BẰNG ĐIỂM → kích hoạt Tie-Breaking.
    ...
    public List<TeamRankingResponse> getRankingForRound(Long roundId) {
        ...
        List<Submission> activeSubmissions = submissions.stream()
                .filter(s -> s.getTeam() != null &&
                        !"DISQUALIFIED".equals(s.getTeam().getStatus()))
        ...
        List<TeamRankingResponse> rankings = scoresBySubmission.entrySet().stream()
                .map(entry -> {
                    ...
                    return TeamRankingResponse.builder()
                            ...
                            // submittedAt = thời điểm nộp/cập nhật bài gần nhất (Last Submit)
                            // Được cập nhật mỗi lần team update submission (không phải lần đầu tiên)
        ...
        for (List<TeamRankingResponse> trackRankings : groupedByTrack.values()) {
            ...
            trackRankings.sort((r1, r2) -> {
                ...
                // submittedAt được cập nhật mỗi lần team update bài — không phải lần nộp đầu tiên
        ...
        finalRankings.sort(Comparator.comparing((TeamRankingResponse r) ->
                        r.getTrackId() != null ? r.getTrackId() : -1L)
